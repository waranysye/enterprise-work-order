...
export default async function WorkOrderEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const serializedWorkOrder = workOrder
    ? (JSON.parse(JSON.stringify(workOrder)) as SerializedWorkOrder)
    : null;
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm shadow-slate-900/5">
          <div className="flex items-center gap-2 text-sm text-slate-500">
            ...
            <span>/</span>
            <Link href={`/work-orders/${workOrder.id}`} className="hover:text-blue-600 transition-colors truncate max-w-[200px]">
              {workOrder.title}
            </Link>
            <span>/</span>
            <span className="text-slate-700">Edit</span>
          </div>
          <h1 className="mt-3 text-2xl font-semibold text-slate-900">Edit Work Order</h1>
          <p className="mt-1 text-sm text-slate-600">Perbarui detail work order sesuai kebutuhan.</p>
        </div>

        {/* Form */}
        <div className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm shadow-slate-900/5">
          <WorkOrderForm initial={serializedWorkOrder ?? undefined} />
        </div>
      </div>
    </AppShell>
  );
